...
위 주제로 수능 국어 비문학 지문을 작성하세요.

반드시 다음의 JSON 형식으로만 답변하세요:
{
  "title": "지문 제목 (30자 이내)",
  "category": "지문 분야 (예: 경제, 과학, 사회, 기술, 문화 중 하나)",
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (keyword === '테스트' || keyword === 'mock' || !process.env.GEMINI_API_KEY) {
            return NextResponse.json({ passage_id: 'mock-passage-id', cache_hit: false });
        }
        // -----------------------
        ...
        if (existing) {
            // Cache hit — deduct credit, increment usage, return immediately
            await Promise.all([
                supabase.from('profiles').update({ credits: profile.credits - 1 }).eq('id', user.id),
                supabase.from('passages').update({ usage_count: existing.usage_count + 1 }).eq('id', existing.id),
            ]);
            return NextResponse.json({ passage_id: existing.id, cache_hit: true });
        }
        ...
        const result = await model.generateContent(buildUserPrompt(keyword.trim()));
        ...
        let parsed: { title: string; category: string; content: string; logic_structure: string | string[] };
        try {
        ...
        const { data: newPassage, error: insertError } = await supabase
            ...
            .insert({
                ...
                logic_structure: Array.isArray(parsed.logic_structure)
                    ? parsed.logic_structure.join('\n')
                    : parsed.logic_structure,
                is_public: false,
            })
            .select('id')
            .single();
        ...
        return NextResponse.json({ passage_id: newPassage.id, cache_hit: false });
    } catch (err) {
